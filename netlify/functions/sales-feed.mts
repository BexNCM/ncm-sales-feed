/* =====================================================================
   ...
   Per-rep activity vs prorated targets (part-time weeks supported),
   today/yesterday/this-week, sourced leads (14 days), hammer pipeline
   (Exclusive Proposal-sent + Collective Terms-sent, <90 days) with
   per-deal detail + missing-financial flags, LotOut costing
   cross-reference, terms sent (Exclusive + Collective), 6-week trends,
   Activity detail with call direction. Rate-limited under HubSpot's
   4/sec cap; result cached 4 min. ?diagnostics=1 checks credentials;
...
const REPS = [
  { id:"29383896",  name:"Neil Rayner",    initials:"NR", role:"National Business Development Manager", daysPerWeek:5, offWeekday:null },
  { id:"896985590", name:"Amy Rutherford", initials:"AR", role:"Partnerships Lead", daysPerWeek:5, offWeekday:null },
  { id:"897221581", name:"Emma Humble",    initials:"EH", role:"Head of Partnerships", daysPerWeek:4, offWeekday:5 }
];
const SOURCERS = [
...
function workingDaysBetween(from, to, off){
  let d = new Date(from), n = 0;
  while (d < to){ const w = d.getUTCDay(); if (w!==0 && w!==6 && w!==off) n++; d = new Date(d.getTime()+86400000); }
  return Math.max(n,1);
}
...
export default async (req) => {
  ...
  try {
    ...
    for (const rep of REPS){
      ...
      const repWorkingDays = workingDaysBetween(sinceMs, now, rep.offWeekday);
      const repWeeks = repWorkingDays / rep.daysPerWeek;
      const cm = callMetrics(calls, companyMap, repWorkingDays, repWeeks);
